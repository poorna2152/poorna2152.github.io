...
Subset02 of the language contains the type `any`. A `boolean`, `integer` or `nil` value can be assigned to a variable of type `any` and a value assigned to an `any` can be casted back to its original type. 
